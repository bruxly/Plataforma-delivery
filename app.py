...
from firebase_admin import credentials, firestore,auth, firestore, initialize_app
...
if "has_run" not in st.session_state:
    ...
    st.session_state.redirect_uri = "https://plataforma-delivery-casanareyopal.streamlit.app"########################
    ############################################################################
    ####################################################################
...
def exchange_code_for_tokens(auth_code):
    ...
    if response.status_code == 200:  # si la respuesta es exitosa
        ...
        st.error(
            f"Error al obtener el token: {response.text}")  # si hay un error, muestra el mensaje de error
...
# Verificar si existe algun pedido en el carrito de compras
def get_latest_cart(session_id):
    try:
        doc = st.session_state.db.collection('carts').document(session_id).get()
        
        # Verificar si existe el documento
        if doc.exists:
            data = doc.to_dict()
            st.session_state.cart = []
            firestore_items = data.get('items', [])
            ...
            return True  # Indica que se cargó exitosamente
        else:
            return False  # No existe el documento
    except Exception as e:
        st.error(f"Error al obtener el carrito: {e}")
        return False
...
if "payment" in query_params:
    if query_params["payment"] == "success":  # si el pago fue exitoso
        ...
        st.session_state.usuario = get_user_from_firestore(st.session_state['stripe_session_id'])
        st.session_state.login = True
        load_cart = get_latest_cart(st.session_state['stripe_session_id'])
        if load_cart: st.warning("⚠️ El pago fue cancelado, pero puedes continuar comprando.")
        st.query_params.clear()
        st.switch_page("pages/catalogo.py")  # fuerza la aplicacion a reiniciarse desde elprincipio, despues de limpiar parametros o cambiar de estado
